...
**What this is:** Practice exercises against the running app. No answers, no hidden `tests/` tree.  
**Not this:** What the product must do ([../PRODUCT_REQUIREMENTS.md](../PRODUCT_REQUIREMENTS.md)) or how to structure the suite ([FRAMEWORK.md](./FRAMEWORK.md)).

Write tests against the **running app** and [API.md](../API.md). Accounts, seed ids, and expected oracles: [README — Test data](../../README.md#test-data-manual-kit). How to design the suite: [README.md](./README.md). If a challenge mentions a feature that is not built yet, wait for that iteration.
